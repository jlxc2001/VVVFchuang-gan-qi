...
V11 在手机传感器模式里加入了停车状态机，专门解决“只靠加速度积分，停车后速度不归零”的问题。

状态流转大致为：

```text
STOPPED → ACCELERATING → CRUISING → BRAKING → STOP_CANDIDATE → STOP_CONFIRMED
```

核心逻辑：

- 向前加速度超过阈值后，认为车辆起步/加速，并自动锁定手机前进轴向。
- 加速度回到接近 0 时，不立刻归零，而是进入巡航，表示车辆可能在匀速行驶。
- 检测到持续向后加速度时，进入刹车状态，并累计“刹车冲量”。
- 刹车结束后，如果加速度稳定、估算速度足够低，或 GPS 低速辅助确认，就进入“疑似停车”。
- 疑似停车满足稳定时间后，目标速度置 0，但仍然经过本项目原有的音频平滑层过渡，不会咔一下切断声音。
- 融合模式下，GPS 只作为慢校正和低速确认；如果加速度已经确认停车，几秒内的 GPS 延迟非零速度不会马上把声音重新拉起来。
...
纯加速度模式也能检测刹停，但没有绝对速度锚点，长期行驶仍可能漂移；融合模式更适合真实上车使用。
